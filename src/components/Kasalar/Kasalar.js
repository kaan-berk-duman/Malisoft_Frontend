...
const Tablo = () => {
  ...
  const fetchKasalar = useCallback(async () => {
    try {
      const response = await axios.get("http://localhost:5127/api/kasalar/list");
  ...
  const exportToExcel = () => {
    ...
    const formattedData = kasalar.map(
      ({
        kasa_id,
        kasa_kodu,
        kasa_adi,
        ...
        "Kasa Kodu": kasa_kodu,
        "Kasa Adı": kasa_adi,
        "Kasa Döviz Tipi": kasa_doviz_tipi,
        "Bakiye": formatMaas(bakiye),
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Kasalar");
    XLSX.writeFile(workbook, "Kasalar.xlsx");
  };
  ...
  const exportToPDF = () => {
    ...
    doc.save("Kasalar.pdf");
  };

  const filteredKasalar = searchTerm
    ? kasalar.filter((kasa) => {
      return (
        (kasa.kasa_kodu && kasa.kasa_kodu.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (kasa.kasa_adi && kasa.kasa_adi.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (kasa.kart_durumu && kasa.kart_durumu.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    })
    : kasalar;

  const totalPages = Math.ceil(filteredKasalar.length / recordsPerPage);
  const currentDepolar = filteredKasalar.slice(
            ...
            <table className="min-w-full bg-white border-collapse printable-area">
              <thead>
                <tr>
                  ...
                  <th className="text-left py-2 px-4 border-b-2 border-gray-200 text-gray-600 font-semibold" style={{ pointerEvents: "none" }}>Kasa Kodu</th>
                  <th className="text-left py-2 px-4 border-b-2 border-gray-200 text-gray-600 font-semibold" style={{ pointerEvents: "none" }}>Kasa Adı</th>
                  <th className="text-left py-2 px-4 border-b-2 border-gray-200 text-gray-600 font-semibold" style={{ pointerEvents: "none" }}>Kasa Döviz Tipi</th>
                  <th className="text-left py-2 px-4 border-b-2 border-gray-200 text-gray-600 font-semibold" style={{ pointerEvents: "none" }}>Bakiye</th>
              ...
              <tbody>
                {currentDepolar.length > 0 ? (
                  currentDepolar.map((kasa) => (
                    <tr key={kasa.depo_id} className="hover:bg-gray-100">
                      <td className="py-2 px-4 border-b border-gray-200" style={{ pointerEvents: "none" }}>{kasa.kasa_id}</td>
                      <td className="py-2 px-4 border-b border-gray-200" style={{ pointerEvents: "none" }}>{kasa.kasa_kodu}</td>
                      <td className="py-2 px-4 border-b border-gray-200" style={{ pointerEvents: "none" }}>{kasa.kasa_adi}</td>
                      <td className="py-2 px-4 border-b border-gray-200" style={{ pointerEvents: "none" }}>{kasa.kasa_doviz_tipi}</td>
                      <td className="py-2 px-4 border-b border-gray-200" style={{ pointerEvents: "none" }}>{kasa.bakiye}</td>
                      <td className="py-2 px-4 border-b border-gray-200" style={{ pointerEvents: "none" }}>{kasa.kart_durumu}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
            ...
            <div className="flex justify-between items-center p-4 border-t">
              ...
              <div className="flex items-center gap-4">
                <span className="text-sm font-medium text-gray-700" style={{ pointerEvents: "none" }}>
                  ...
                  {Math.min(currentPage * recordsPerPage, filteredKasalar.length)} of {filteredKasalar.length}
                </span>
